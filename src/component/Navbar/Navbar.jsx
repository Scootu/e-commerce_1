...
export default function NavBar() {
  ...
  useEffect(() => {
    // Accessing the element and getting its position
    const navbar = navbarRef.current;
    const navPos = navbar.getBoundingClientRect().top;
    let scroll = 0;
    // Example: Add a scroll event listener
    const handleScroll = () => {
      // Do something on scroll, e.g., change styles based on scroll position

      let scrollPos = window.scrollY;
      console.log("scroll :", scroll, "scrollPos", scrollPos);
      let isScrollUp = scrollPos < scroll - 100;
      if (isScrollUp && scrollPos > 100) {
        setIsSticky(true);
